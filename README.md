...
- ✅ **Native ABI Drift Fixed for Desktop Builds** - Windows packaging now stages an Electron ABI prebuilt `better-sqlite3` binary (`electron:prepare-better-sqlite3`) while disabling broad dependency rebuilds (`npmRebuild: false`) to avoid Windows SDK/toolchain drift failures.
