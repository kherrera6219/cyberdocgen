...
// Isolate PGlite data directory per test file to prevent corruption and state leakage
process.env.LOCAL_DATA_PATH = `./.pgdata-test-${process.env.VITEST_POOL_ID || process.env.VITEST_WORKER_ID || '0'}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
