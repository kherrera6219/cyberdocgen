...
export async function generateContentWithGemini(
  ...
  try {
    ...
    if (model === GEMINI_PRO_MODEL) {
      logger.warn("Gemini 2.5 Pro failed, falling back to Gemini 2.0 Flash", { error });
      try {
        const client = getSharedGeminiClient();
        const response = await client.models.generateContent({
        ...
        throw new Error("Failed to generate content with Gemini (both Pro and Flash unavailable)");
      }
    }
