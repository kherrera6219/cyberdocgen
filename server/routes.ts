...
Category: ${category}`;
  ...
  // AI Document Generation with company profile from wizard
  app.post('/api/ai/generate-compliance-docs', isAuthenticated, generationLimiter, async (req: any, res) => {
    try {
      ...
      // Get user's organization (or create default one)
      ...
      // Create a real company profile with framework-specific configurations
      ...
      // Create generation job with real profile ID
      const job = await storage.createGenerationJob({
      ...
      // Background generation with validation
      (async () => {
        try {
          const generatedDocs: any[] = [];
          
          for (const framework of frameworks) {
            // Use profile with framework configs for better prompts
            const profileForAI = {
              ...companyProfile,
  ...
  // Get generation job status for wizard polling
  app.get('/api/ai/generation-jobs/:id', isAuthenticated, async (req: any, res) => {
    try {
      const job = await storage.getGenerationJob(req.params.id);
      if (!job) {
