...
export function RiskHeatmap({ className }: RiskHeatmapProps) {
  ...
  const getRiskIcon = (level: string) => {
    switch (level) {
      ...
      case "high": return <AlertTriangle className="h-4 w-4 text-orange-600 dark:text-orange-400" aria-hidden="true" />;
      case "critical": return <AlertTriangle className="h-4 w-4 text-red-600 dark:text-red-400" aria-hidden="true" />;
      default: return null;
    }
  };
              ...
              {categories.map(category => (
                <tr key={category} role="row">
                  ...
                  {frameworks.map(framework => {
                    ...
                    if (!cell) return <td key={framework} role="gridcell" />;
                    
                    const cellAriaLabel = `${category} for ${framework}: ${cell.riskLevel} risk, ${cell.controlsImplemented} of ${cell.totalControls} controls implemented`;
                    
                    return (
                      <td key={framework} className="py-2 px-2" role="gridcell">
                        <Tooltip>
          ...
          <div className="flex items-center justify-between text-sm flex-wrap gap-2">
            ...
            <div className="flex items-center gap-4 flex-wrap" role="list">
              <div className="flex items-center gap-2" role="listitem">
                <div className={`w-3 h-3 rounded ${getRiskColor("low")}`} aria-hidden="true" />
                <span className="text-gray-600 dark:text-gray-300">Low (80%+)</span>
              </div>
              <div className="flex items-center gap-2" role="listitem">
                <div className={`w-3 h-3 rounded ${getRiskColor("medium")}`} aria-hidden="true" />
                <span className="text-gray-600 dark:text-gray-300">Medium (60-79%)</span>
              </div>
              <div className="flex items-center gap-2" role="listitem">
                <div className={`w-3 h-3 rounded ${getRiskColor("high")}`} aria-hidden="true" />
                <span className="text-gray-600 dark:text-gray-300">High (40-59%)</span>
              </div>
              <div className="flex items-center gap-2" role="listitem">
                <div className={`w-3 h-3 rounded ${getRiskColor("critical")}`} aria-hidden="true" />
